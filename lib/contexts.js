...
class Context {
  ...
  _processV2InputContexts(v2InputContexts) {
    ...
    for (let index = 0; index<v2InputContexts.length; index++) {
      ...
      let name;
      if (context['name'].indexOf('/agent/environments/') !== -1) {
        // Ex. projects/<Project ID>/agent/environments/<Environment ID>/users/-/sessions/<Session ID>/contexts/<Context ID>
        name = context['name'].split('/')[10];
      } else {     
        // Ex. projects/<Project ID>/agent/sessions/<Session ID>/contexts/<Context ID>
